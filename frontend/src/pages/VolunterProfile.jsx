...
// Importamos la función updateUsuario que ahora debe manejar FormData
...
export default function VolunterProfile() {
  ...
  // Mantenemos el estado para el archivo seleccionado, pero eliminamos isUploading
  ...
  // Función auxiliar para construir la URL de la foto de perfil (similar al PsicologoProfile)
  ...
  // FUNCIÓN MODIFICADA: Ahora maneja tanto los campos de texto como la subida de la foto
  const handleUpdate = async (e) => {
    ...
    for (const key in formData) {
      // Ignorar campos que no deben enviarse para actualización
      if (key === 'fecha_nacimiento') continue; // No editable
      if (key === 'email') continue; // Asumo que el email no se edita aquí o tiene otra validación
    ...
    // 3. ESTRATEGIA DE ENVÍO: Usar FormData si hay archivo
    let requestData;
    if (profileImageFile) {
      // Crear FormData para enviar el archivo y los datos de texto
    ...
    try {
      ...
      // Actualizar el contexto del usuario con la nueva información si aplica
      if (loggedInUser.id.toString() === id) {
        setUsuario(prev => ({
            ...prev,
            nombre: res.data.nombre,
            apellido: res.data.apellido,
            fotoUrl: res.data.fotoUrl || prev.fotoUrl // El backend debe devolver la nueva fotoUrl
        }));
      }

      toast({ title: "Perfil actualizado 🎉", status: "success" });
      setIsEditing(false);
      setProfileImageFile(null); // Limpiar el estado del archivo
      
      // Recargar para actualizar los datos mostrados
      fetchVoluntario(); 
  ...
  // FUNCIÓN: Para manejar la selección del archivo (similar a la anterior, solo cambiamos el nombre para consistencia)
  const handleFileChange = (e) => {
    setProfileImageFile(e.target.files[0]);
  };
  
  // ELIMINAMOS la función handleUploadImage, ya no es necesaria.
    ...
    <Box maxW="600px" mx="auto" p={6}>
      ...
      <Box p={6} borderWidth={1} borderRadius="lg" bg="white">
        <Stack align="center" mb={6}>
          ...
          {loggedInId === profileId && isEditing && (
            <Text mt={2} fontSize="sm" color="gray.500">Subir nueva foto abajo</Text>
          )}
              ...
              {loggedInId === profileId && (
                <FormControl>
                  ...
                  <Input
                    ...
                    p={1} // Estilo para que el input se vea mejor
                  />
...
export default function VolunterProfile() {
  ...
  const profileId = id.toString();
  const loggedInId = loggedInUser?.id?.toString();
  // 🔑 Usamos una variable clara para la condición: ¿Es el dueño del perfil?
